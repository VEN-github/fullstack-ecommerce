      ...
      <table class="datatable">
        ...
        <tbody>
          <?php foreach ($suppliers as $supplier): ?>
            ...
            <div id="deleteModal-<?= $supplier->id ?>" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
              <div class="relative p-4 w-full max-w-xl">
                <div class="relative p-4 text-center bg-white rounded-lg shadow sm:p-5">
                  <button data-modal-toggle="deleteModal-<?= $supplier->id ?>" type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="deleteModal">
                  ...
                  <p class="mb-4 text-gray-700">Are you sure you want to delete supplier: <span class="font-semibold text-gray-900"><?= $supplier->name ?></span>?</p>
                  <div class="flex justify-center items-center space-x-4">
                    <button data-modal-toggle="deleteModal-<?= $supplier->id ?>" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-gray-900 focus:z-10">
                      No, cancel
                    </button>
                    <a href="/admin/supplier/<?= $supplier->id ?>/delete" data-modal-toggle="deleteModal-<?= $supplier->id ?>" class="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700">Yes, I'm sure</a>
                  </div>
                </div>
              </div>
            </div>
